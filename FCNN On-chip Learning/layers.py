...
def affine_backward(dout, cache, quant_params):
    ...
    if quant_params is not None:
        ws, _, _, InputStep, threshold1, threshold2, _, _, _, _, _, _, device_var, sigma_d2d, error_d2d_msb_W1, error_d2d_msb_W2, error_d2d_lsb_W1, error_d2d_msb_W2, error_d2d_msb_b1, error_d2d_msb_b2, error_d2d_lsb_b1, error_d2d_lsb_b2, cycle_var_add, device_var_add = quant_params
        if type(ws) is tuple:
            _, WeightStep = ws
        else:
            WeightStep = ws
        # Quantizing and thresholding dout (don't worry about the long formula)
        dout_part1 = WeightStep * (np.sign(0.5 * (((dout - threshold1) * (WeightStep * np.ones(
            dout.shape))) + (np.absolute((dout - threshold1) * (WeightStep * np.ones(dout.shape)))))))
        dout_part2 = WeightStep * (np.sign(0.5 * (((dout + threshold1) * (WeightStep * np.ones(
            dout.shape))) - (np.absolute((dout + threshold1) * (WeightStep * np.ones(dout.shape)))))))
        dout = dout_part1 + dout_part2
        dx = dout.dot(w.T)

        # Quantizing and thresholding the input
        x_part1 = InputStep * (np.sign(0.5 * (((x - threshold2) * (InputStep * np.ones(
            x.shape))) + (np.absolute((x - threshold2) * (InputStep * np.ones(x.shape)))))))
        x_part2 = InputStep * (np.sign(0.5 * (((x + threshold2) * (InputStep * np.ones(
            x.shape))) - (np.absolute((x + threshold2) * (InputStep * np.ones(x.shape)))))))
        x = x_part1 + x_part2
